...
optall = (
    True  # True: use all events for finding rotation, False: use only reference events
)
...
bary = np.array(
    [LLtoUTM(23, 38.4773, -112.8668)[1], LLtoUTM(23, 38.4773, -112.8668)[2], 973.1]
)

wdepths = pd.read_csv("./data/inputs/w78_UTM.csv", delimiter=";")

well_coords_sph = np.array(
    [UTMtoLL(23, wdepths.NS[i], wdepths.EW[i], "12S") for i in range(len(wdepths))]
)
...
for i in tqdm(range(len(stations))):
    print(f"Working on {i} of {len(stations)}")
    ...
    hloc = hades_location(
        input_obj=hobj, output_path=output_path, output_frame="latlon"
    )
    ...
    tstp = np.array(
        [hloc.evtsps[stations[i][0]], hloc.evtsps[stations[i][1]]]
    )  # + np.random.normal(0, 0.01, (3,50))
    dtps_obs = tstp.copy()  # observed ts-tp of n events shape: [n]
    stavect = np.array(
        [w_coords_UTM[channel_pairs[i][0], :], w_coords_UTM[channel_pairs[i][0], :]]
    )

    cl = huge_locmat[:, :, i].copy()

    bary, v0, v1, v2, cluster_opt, dtps_obs, stavect = ru.setup(
        optall,
        master_evts,
        None,
        dtps_obs,
    ...
    spinner.start()
    test_thetas = instance_cluster.optimize_rotations(
        prior=[0.0, 0.0, 0.0], method="diffev", optall=optall
    )
    spinner.stop()
...
np.savetxt(
    "./data/outputs/best_run_EQ_SHORT.csv", huge_locmat[:, :, best_run], delimiter=";"
)
...
np.savetxt(
    "./data/outputs/master_evt_EQ_SHORT.csv", huge_locmat[ref, :, 0], delimiter=";"
)
...
h, _, _, im = ax1.hist2d(
    ...
    vmin=0,
    vmax=100,
    range=([336000, 338500], [4.2595e6, 4.2620e6]),
)
ax1.scatter(
    cluster_comp[ref[0], 0],
    cluster_comp[ref[0], 1],
    s=75,
    edgecolor="orange",
    facecolor="yellow",
...
plt.colorbar(im, ax=ax1, shrink=0.5, label="Location count")
...
for area in [30, 200, 300]:
    ax1.scatter(
        [], [], c="k", alpha=0.3, s=area, label="$\sigma$ = " + str(area) + " m"
    )
ax1.legend(
    scatterpoints=1, frameon=True, labelspacing=1, title="City Area", loc="upper right"
)
...
aa, _, _, im = ax3.hist2d(
    ...
    cmap=cmap,
    vmax=100,
    range=([4.2595e6, 4.2620e6], [0, 2500]),
    vmin=0,
)
...
stations = w_coords_UTM[
    channel_pairs[best_run], :
]  # m stations, lat/lon/depth shape [m, 3]
station_axis = (
    0  # choose index of the station by which you want to define your rotation axis
)

sta1 = stations[0]
sta2 = stations[1]
bary, v0, v1, v2, cluster_opt, dtps_obs, stavect = ru.setup(
    optall, master_evts, None, dtps_obs, cl, stations, station_axis, vs, vp, baryp=bary
)

instance_cluster_refine = Cluster(
    cluster=cl,
    stations=np.array(stavect),
    axis=station_axis,
    bary=bary,
...
spinner.start()
test_thetas_refine = instance_cluster_refine.optimize_rotations(
    prior=[0.0, 0.0, 0.0], method="diffev", optall=optall
)
spinner.stop()
print("Optimizer REFINER outcome in degrees (three rotations)", test_thetas_refine)

cluster_comp_refine = instance_cluster_refine.invert_rotations_spatial(
    cl, test_thetas_refine
)
...
ax.scatter(
    cl[:, 0], cl[:, 1], facecolor="None", edgecolor="darkgreen", label="cluster_rot"
)
...
ax.scatter(
    cl[:, 0], cl[:, 2], facecolor="None", edgecolor="darkgreen", label="cluster_rot"
)
...
plt.grid()
ax.scatter(
    cl[:, 1], cl[:, 2], facecolor="None", edgecolor="darkgreen", label="cluster_rot"
)
...
al_UTM = np.array(
    [LLtoUTM(23, AL_sph.lat[i], AL_sph.lon[i])[1:] for i in range(len(AL_sph))]
)
...
plt.grid()
plt.scatter(
    huge_locmat[:, 0, best_run], huge_locmat[:, 1, best_run], s=(tstp[0] * 10) ** 2
)
plt.scatter(
    huge_locmat[ref, 0, best_run], huge_locmat[ref, 1, best_run], c="yellow", zorder=10
)
plt.scatter(
    huge_locmat[ref[0], 0, best_run], huge_locmat[ref[0], 1, best_run], c="yellow"
)
