...
# Date: 22 Apr 2024
# File: hades_input.py
# Description: This file contains the implementation of the `hades_input` class,
#              which prepares earthquake data for the HADES-R location class.
...
class hades_input:
    ...
    def __init__(self, data_path, event_file, station_file, sta_select):
        ...
        references, refevid, evtsp, reforig, events = hades_input.__read_evfile(
            event_file
        )
        ...
        self.stations = {
            f"{sta_select[0]}": stations[sta_select[0]],
            f"{sta_select[1]}": stations[sta_select[1]],
        }
        # print(self.stations)
            ...
            for line in f:
                ...
                if toks[0][0] == "#":
                    ...
                    if toks[1] == "na" or toks[2] == "na":
                        continue
                    tp = datetime.datetime.strptime(
                        evdate + "T" + toks[1], "%Y/%m/%dT%H:%M:%S.%f"
                    )
                    ts = datetime.datetime.strptime(
                        evdate + "T" + toks[2], "%Y/%m/%dT%H:%M:%S.%f"
                    )
            ...
            for i in range(nevs - 1):
                for j in range(i + 1, nevs):
                    if (i < nref) and (j < nref):
                        distances[i, j] = num.sqrt(
                            (evrefs[i][0] - evrefs[j][0]) ** 2
                            + (evrefs[i][1] - evrefs[j][1]) ** 2
                            + (evrefs[i][2] - evrefs[j][2]) ** 2
                        )
                    else:
                        tsp_ev1 = self.data[events[i]]
                        tsp_ev2 = self.data[events[j]]
                        distances[i, j] = hades_input.__interev_distance(
                            tsp_ev1, tsp_ev2, kv, sta, self.stations
                        )
    ...
    def relative_frame(self, Vp, Vs, sta, y_ref=-1, z_ref=-1, fixed_depth=0):
        ...
        if len(self.refevid) > 4:
            print(
                "For relative frame construction you need only 4 events, you selected : ",
                len(self.refevid),
            )
            sys.exit()
        else:
        ...
        for i in range(3):
            ...
            for j in range(i + 1, 4):
                tsp_ev2 = self.data[events[j]]
                d[i, j] = hades_input.__interev_distance(
                    tsp_ev1, tsp_ev2, kv, sta, self.stations
                )
                d[j, i] = d[i, j]
        ...
        references[2, 0] = (d[0, 2] ** 2 - d[1, 2] ** 2) / (2 * references[1, 0]) + (
            references[1, 0] / 2
        )
        references[2, 1] = y_ref * num.sqrt(d[0, 2] ** 2 - references[2, 0] ** 2)
        references[2, 2] = 0.0

        references[3, 0] = (d[0, 3] ** 2 - d[1, 3] ** 2) / (2 * references[1, 0]) + (
            references[1, 0] / 2
        )
        references[3, 1] = (
            d[1, 3] ** 2
            - d[2, 3] ** 2
            - (references[3, 0] - references[1, 0]) ** 2
            + (references[3, 0] - references[2, 0]) ** 2
        ) / (2 * references[2, 1]) + (references[2, 1] / 2)

        if fixed_depth:
            references[3, 2] = fixed_depth * 1000.0
        else:
            # TODO: future IMPLEMENT A least-squares misfit solution here!!!
            ...
            for i in range(len(zs)):
                references[3, 2] = (
                    ...
                    * z_ref
                    * num.sqrt(
                        d[0, 3] ** 2 - references[3, 0] ** 2 - references[3, 1] ** 2
                    )
                )
