...
# Date: 22 Apr 2024
# File: hades_location.py
# Description: This file contains the implementation of the `hades_location` class.
...
class hades_location(object):
    ...
    def __init__(self, input_obj, output_path, output_frame, verbose=False):
        ...
        if type(output_frame) != str:
            raise_error(
                "Wrong output_frame type, should be `str` options are cart or latlon"
            )
    ...
    def location(self, filename, mode, plot):
        ...
        if mode == "rel":
            ...
            rect = self.__pca_theta_calculation(
                xobs=self.locations[:, 0],
                yobs=self.locations[:, 1],
                zobs=self.locations[:, 2],
    ...
    def __dgslocator(event, references, distances):
        ...
        for i in range(n_ref):
            for j in range(n_ref):
                ...
                Zi = references[i, 2]
                Zj = references[j, 2]
                dio = distances[i, event]
                djo = distances[j, event]
                dij = num.sqrt((Xi - Xj) ** 2 + (Yi - Yj) ** 2 + (Zi - Zj) ** 2)
                # dij=distances[i,j]
                D[i, j] = (dio**2 + djo**2 - dij**2) / 2.0
                D[j, i] = D[i, j]
    ...
    def __absolute_cluster_location(self, filename, plot, acc=1):
        ...
        def define_axis_vectors(sta, bary):
            ...
            vec1 = num.array(
                [sta[0] - bary[0], sta[1] - bary[1], 0]
            )  # sta1-barycenter vector
            vec2 = num.cross(vec0, vec1)

            n0, n1, n2 = (
                num.linalg.norm(vec0),
                num.linalg.norm(vec1),
                num.linalg.norm(vec2),
        ...
        def z_quat(theta):
            ...
            r_a = R.from_quat(
                [
                    v0[0] * num.sin(theta / 2),  # compute quaternions
        ...
        def a_quat(theta):
            ...
            r_b = R.from_quat(
                [
                    v1[0] * num.sin(theta / 2),  # compute quaternions
        ...
        def b_quat(theta):
            ...
            r_c = R.from_quat(
                [
                    v2[0] * num.sin(theta / 2),  # compute quaternions
        ...
        for ysign in [-1, 1]:
            for theta in thetas:
                crot = (
                    self.locations[:, 0] + 1j * ysign * self.locations[:, 1]
                ) * num.exp(-1j * theta)
                rms = self.__rms_theta_calculation(
                    crot.real, crot.imag, zrot, evtsps, kv, stations
                )
                if rms < rms_min:
                    rms_min = rms
                    theta_best = theta
                    ysign_best = ysign
        crot = (
            self.locations[:, 0] + 1j * ysign_best * self.locations[:, 1]
        ) * num.exp(-1j * theta_best)
        ...
        with open(os.path.join(self.output_path, (filename + ".txt")), "w") as f:
            ...
            for i in range(nev):
                ...
                if self.output_frame == "latlon":
                    lat, lon, _ = orig.cart2geo(
                        self.locations[i, 0], self.locations[i, 1], 0
                    )
                ...
                event = evids[i]
                evtno = int(
                    evids[i].split("#")[-1].split("R")[-1].split("A")[-1].split("E")[-1]
                )
                t_string = ""
    ...
    def __plot_results(self, filename):
        ...
        for sta in (self.input).stations.keys():
            ...
            ax1.scatter(
                station[0], station[1], c=c2, marker="v", s=200, zorder=3, linewidth=0.5
            )
        ax1.grid("on")
