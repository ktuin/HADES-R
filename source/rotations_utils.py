...
"""A set of functions that could be used in the Cluster class, as well as separately."""

def setup(optall, master_evts, cluster, dtps_obs, cluster_rot, stations, station_axis, vs, vp, baryp=[0,0,0]):
    ...
    v0, v1, v2 = define_axis_vectors(sta=stations[station_axis], bary=bary)  # axis unit vectors
    #------------------------#
    ...
    if optall==False: # create a subcluster of master events
        ...
        for o in range(len(stavect)):
            _, _,  ttp_org, tts_org = compute_traveltime(cluster=subcluster_org, stat=stavect[o], vp=vp, vs=vs, frame=False, noise=0.0)
            dtps_obs[o,:] = np.array([tts_org - ttp_org]) # tt differences between s and p

        cluster_opt = subcluster_rot
        print(f"Optall = {optall}, use only master events for rotation finder")

    elif optall==True:
        stavect = stations#[sta1, sta2, sta3, sta4, sta5, sta6]
        dtps_obs = dtps_obs.copy()
#         dtps_obs = np.zeros([len(stavect), len(cluster_rot)])

#         for o in range(len(stavect)):
#             _, _,  ttp_org, tts_org = compute_traveltime(cluster=cluster, stat=stavect[o], vp=vp, vs=vs, frame=False, noise=0.0)
#             dtps_obs[o,:] = np.array([tts_org - ttp_org]) # tt differences between s and p
...
def compute_distance(x_c, x_r, frame):
        ...
        if frame==True:     # if in latlon convert to a UTM domain for cartesian distance
            ...
            for i in range(len(cluster)):
                x_cutm = LLtoUTM(23, x_c[:,1], x_c[:,0])
                x_rutm = LLtoUTM(23, x_r[1], x_r[0])
...
def test_cluster_rotation(cluster, thetas, deg, v0, v1, v2, bary):
    if deg == True:
        thetas = np.array([np.radians(thetas[0]), np.radians(thetas[1]), np.radians(thetas[2])])

    # Compute the quaternions
    r_1 = R.from_quat([v0[0]*np.sin(thetas[0]/2), v0[1]*np.sin(thetas[0]/2), v0[2]*np.sin(thetas[0]/2), np.cos(thetas[0]/2)])
    r_2 = R.from_quat([v1[0]*np.sin(thetas[1]/2), v1[1]*np.sin(thetas[1]/2), v1[2]*np.sin(thetas[1]/2), np.cos(thetas[1]/2)])
    r_3 = R.from_quat([v2[0]*np.sin(thetas[2]/2), v2[1]*np.sin(thetas[2]/2), v2[2]*np.sin(thetas[2]/2), np.cos(thetas[2]/2)])
...
def define_axis_vectors(sta, bary):
    ...
    n0, n1, n2 = np.linalg.norm(vec0), np.linalg.norm(vec1), np.linalg.norm(vec2)   # norms of the vectors
    v0 = np.array([vec0[0]/n0, vec0[1]/n0, vec0[2]/n0])            # convert into unit vectors
...
def plot_clusters_3D(cluster, sta1, sta2, bary):
    ...
    ax.scatter(sta1[0], sta1[1], sta1[2], marker='v', color='red', s=50, label='Station 1')
    ax.scatter(sta2[0], sta2[1], sta2[2], marker='v', color='darkred', s=50, label='Station 2')

    ax.scatter(bary[0], bary[1], bary[2], color='cyan', label='barycenter')

    ax.plot(xs=[bary[0], sta1[0]], ys=[bary[1],sta1[1]], zs=[bary[2],sta1[2]], color='grey', label='Axis')
    ax.plot(xs=[bary[0], sta2[0]], ys=[bary[1],sta2[1]], zs=[bary[2],sta2[2]], color='grey')
    ax.plot(xs=[bary[0], bary[0]], ys=[bary[1],bary[1]], zs=[bary[2], 0.0], color='grey')

    ax.plot(xs=[bary[0], sta1[0]], ys=[bary[1],sta1[1]], zs=bary[2], color='grey', linestyle='--', 
            label='projected axis')
    ax.plot(xs=[bary[0], sta2[0]], ys=[bary[1],sta2[1]], zs=bary[2], color='grey', linestyle='--')
...
def plot_cluster_misfit_3D(cluster, misfit, **kwargs):
    ...
    if 'vmin' in kwargs or 'vmax' in kwargs:
        im = ax.scatter(cluster[:,0], cluster[:,1], cluster[:,2], c=misfit, s=75, 
                        cmap='bwr', label='cluster', vmin=kwargs['vmin'], vmax=kwargs['vmax'])
    else:
        minmax = np.max([-misfit.min(), misfit.max()])
        im = ax.scatter(cluster[:,0], cluster[:,1], cluster[:,2], c=misfit, s=75, 
                        cmap='bwr', label='cluster', vmin=-minmax, vmax=minmax)


    ax.set_xlabel('X'), ax.set_ylabel('Y'), ax.set_zlabel('Z')
    plt.colorbar(im, label='Error', orientation='horizontal', shrink=.7)
...
def plot_cluster_tt_3D(cluster, stations, sta, dtps, bary):
    ...
    im = ax.scatter(cluster[:,0], cluster[:,1], cluster[:,2], c=dtps[sta,:], s=75, cmap='plasma', label='cluster')
    ax.scatter(stations[sta, 0], stations[sta,1], stations[sta, 2], marker='v', color='red', s=200)
    ax.scatter(stations[:,0], stations[:,1], stations[:,2], marker='v', color='darkred', s=50)
    ax.plot(xs=[bary[0], stations[sta,0]], ys=[bary[1],stations[sta,1]], zs=[bary[2],stations[sta,2]], color='red')
    ...
    ax.set_xlabel('X'), ax.set_ylabel('Y'), ax.set_zlabel('Z')
    ax.set_zlim(cluster[:,2].min()-.2*cluster[:,2].min(), 1)
    plt.colorbar(im, orientation='horizontal', shrink=.7)
...
def plot_compare_beforeafter(cluster, cluster_rot, cluster_comp):
    ...
    ax1.scatter(cluster[:,0], cluster[:,1], cluster[:,2], s=35, color='black', label='original cluster')
    ax1.scatter(cluster_rot[:,0], cluster_rot[:,1], cluster_rot[:,2], s=35, color='green', marker='p', label='rotated cluster')
    for i in range(len(cluster_rot)):
        ax1.plot([cluster[i,0],cluster_rot[i,0]],
        [cluster[i,1],cluster_rot[i,1]],
        [cluster[i,2],cluster_rot[i,2]], 'cyan', alpha=.5)
    plt.legend()
    ax1.set_xlabel('x'),ax1.set_ylabel('y'),ax1.set_zlabel('z')

    ax2 = fig.add_subplot(122, projection='3d')
    plt.title('After rotation', fontsize=15)
    ax2.scatter(cluster[:,0], cluster[:,1], cluster[:,2], s=35, color='black', label='original cluster')
    ax2.scatter(cluster_comp[:,0], cluster_comp[:,1], cluster_comp[:,2], s=35, color='green', marker='p', label='computed cluster')
    for i in range(len(cluster_comp)):
        ax2.plot([cluster[i,0],cluster_comp[i,0]],
        [cluster[i,1],cluster_comp[i,1]],
        [cluster[i,2],cluster_comp[i,2]], 'cyan')
